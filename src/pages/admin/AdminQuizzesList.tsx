...
import { Plus, Trash2, Edit, ChevronRight, Search } from 'lucide-react';
import { useAdminStore } from '../../store/adminStore';
import { quizCases } from '../../lib/quizCases';
import clsx from 'clsx';

export const AdminQuizzesList: React.FC = () => {
    const navigate = useNavigate();
    const { customQuizzes, deleteQuiz } = useAdminStore();
    const [search, setSearch] = useState('');
    const [showDeleteModal, setShowDeleteModal] = useState<string | null>(null);

    const allQuizzes = [...quizCases, ...customQuizzes];
    const filteredQuizzes = allQuizzes.filter(q =>
        q.correctDiagnosis.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const isCustom = (id: string) => customQuizzes.some(q => q.id === id);
            ...
            <div className="space-y-3">
                    ...
                    return (
                        <div
                            ...
                            className={clsx(
                                ...
                                custom ? "border-emerald-500/30" : "border-slate-700"
                            )}
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <h3 className="font-bold text-white truncate">{quiz.correctDiagnosis}</h3>
                                        {custom && (
                                    ...
                                    <div className="flex items-center gap-3 text-xs text-slate-500">
                                        ...
                                        <span>•</span>
                                        <span>{quiz.hints.length} pistas</span>
                                    </div>
                                </div>
                ...
                {filteredQuizzes.length === 0 && (
                    <div className="text-center py-12">
                        <p className="text-slate-400">Nenhum quiz encontrado</p>
                    </div>
                )}
            </div>
